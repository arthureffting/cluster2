...
import sys
from .storage import store_json
        ...
        for region in xml_data[0]['regions']:
            ...
            for i, line in enumerate(xml_data[0]['lines']):
                ...
                warp_output_file = os.path.join(output_directory, basename,
                                                "{}-{}-{}.png".format(basename, str(region["id"]),
                                                                      str(line["read_order"])))
                warp_output_file_save = os.path.join(basename,
                                                     "{}-{}-{}.png".format(basename, str(region["id"]),
                                                                           str(line["read_order"])))
                save_file = os.path.join(basename, "{}~{}~{}.png".format(basename, "save-" + str(region["id"]),
                                                                         str(line["read_order"])))
                ...
                if not os.path.exists(os.path.dirname(warp_output_file)):
                    try:
                        os.makedirs(os.path.dirname(warp_output_file))
...
def process_pairs(set_name, pairs, output_directory):
    all_ground_truth = []
    ...
    for i, pair in enumerate(pairs):
        sys.stdout.write(
            "\r" + "[Creating ground truth for " + set_name + "] " + str(round(100 * (i + 1) / len(pairs), 2)) + "%")
        img_path = pair.img
        xml_path = pair.processed_xml

        # Skip if the output folder for this pair already exists
        this_output_directory = os.path.join(data_output_directory, str(pair.index))

        if os.path.exists(this_output_directory):
            basename = xml_path.split("/")[-1][:-len(".xml")]
            json_path = os.path.join(data_output_directory, basename, "{}.json".format(basename))
...
def prepare(pairs, output_directory, set_name):
    training_output_json = os.path.join(output_directory, set_name + ".json")
    all_ground_truth = process_pairs(set_name, pairs, output_directory)
    store_json(all_ground_truth, training_output_json)
